...
class Operator(object):
    ...
    def join(self, space, row_basis, col_basis, M):
        ...
        self.index_order.append(
            SpaceIndex(space=space, col=False, N=nR)
        )
        self.index_order.append(
            SpaceIndex(space=space, col=True,  N=nC)
        )
        self.space_segments.append(
            (space,)
        )
    ...
    def segments_set(self, space_segments):
        ...
        assert(set(itertools.chain(*new_segments)) == set(itertools.chain(*self.space_segments)))
        ...
        self.mat = self.mat.reshape(*shape_map).transpose(trans_map).reshape(self.nR, self.nC)
    ...
    def __matmul__(self, other):
        ...
        if cspaces:
            if (set(self.space_segments[-1]) == cspaces) and (self.space_segments[-1] == other.space_segments[-1]):
            ...
            for space in cspaces:
                ...
                cindex_order.append(
                    SpaceIndex(space, col = False, N = sbR.N)
                )
                cindex_order2.append(
                    SpaceIndex(space, col = True, N = obC.N)
                )
            snR = self.nR // nR
            snC = self.nC // nI
            onR = other.nR // nI
    ...
    def T(self):
        ...
        n.nR = self.nC
        n.nC = self.nR
        n.space_map = {space : SpaceBasis(sb.bC, sb.bR) for space, sb in self.space_map.items()}
