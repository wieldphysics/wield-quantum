...
def T_qop_gkp_norm(tpath_join, pprint, plot):
    ...
    psi_p2 = (fock.gkp(q=bQ.bP.p, D=D, mu=0) + fock.gkp(q=bQ.bP.p, D=D, mu=1))/2**0.5
...
def T_qop_gkp_norm2(tpath_join, pprint, plot):
    ...
    pprint(psi.nR, psi.nC, psi.mat.shape)
    #the Fourier transform of a GKP is in the superposition of |0> and |1>
    psi_p2 = np.exp(np.pi/4*1j)*(fock.gkp(q=bQ.bP.p, D=D, mu=0) - 1j*fock.gkp(q=bQ.bP.p, D=D, mu=1))/2**0.5
...
def T_gkp_wigner_span(tpath_join, pprint, plot):
    ...
    for idx, iD in enumerate(iDs):
        ...
        axB.ax0_1.stem(abs(psi_gkp.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
        ...
        axall.stem(abs(psi_gkp.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
...
def T_sqz_wigner(tpath_join, pprint, plot):
    ...
    for idx, db in enumerate(dbs):
        ...
        axB.ax0_1.stem(abs(psi_sqz.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
        axB.ax0_1.set_xscale('log')
        axB.save(tpath_join('wigner_sqz_{:.1f}db'.format(-db).replace('.', 'p')))
        plot_wigner(axBall['ax{}_0'.format(idx)], 'field', psi = F2Q @ psi_sqz, bQ=bQ, lims=10)
        axall = axBall['ax{}_1'.format(idx)]
        axall.stem(abs(psi_sqz.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
        axall.set_xscale('log')
...
def T_shear_wigner(tpath_join, pprint, plot):
    ...
    for idx, db in enumerate(dbs):
        ...
        axB.ax0_1.set_xscale('log')
        axB.save(tpath_join('wigner_sqz_{:.1f}db'.format(-db).replace('.', 'p')))
        plot_wigner(axBall['ax{}_0'.format(idx)], 'field', psi = F2Q @ psi_sqz, bQ=bQ, lims=5)
        axall = axBall['ax{}_1'.format(idx)]
        axall.stem(abs(psi_sqz.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
        axall.set_xscale('log')
    axBall.save(tpath_join('wigner_sqz'.format(-db)))
...
def T_shear_gkp_wigner(tpath_join, pprint, plot):
    ...
    for idx, db in enumerate(dbs):
        ...
        axB.ax0_1.set_xscale('log')
        axB.save(tpath_join('wigner_sqz_{:.1f}db'.format(-db).replace('.', 'p')))
        plot_wigner(axBall['ax{}_0'.format(idx)], 'field', psi = F2Q @ psi_sqz, bQ=bQ, lims=15)
        axall = axBall['ax{}_1'.format(idx)]
        axall.stem(abs(psi_sqz.mat[:, 0])**2, markerfmt='C0,', use_line_collection = True)
        axall.set_xscale('log')
    axBall.save(tpath_join('wigner_sqz'.format(-db)))
