...
def psi_gkp(space, bN, D, mu=0, s=None):
    ...
    if mu in [0, 1]:
        return qop.Operator(
            ...
            bN,
            bI,
            dq**0.5 * fock.gkp(bN.q, D=D, mu=mu, s=s)
        )
    elif mu == '+':
        return qop.Operator(
            space,
            bN,
            bI,
            dq**0.5 * (fock.gkp(bN.q, D=D, mu=0, s=s) + fock.gkp(bN.q, D=D, mu=1, s=s))/2**0.5
        )
    elif mu == '-':
        return qop.Operator(
            space,
        ...
        return qop.Operator(
            ...
            bN,
            bI,
            dq**0.5 * (fock.gkp(bN.q, D=D, mu=0, s=s) + 1j*fock.gkp(bN.q, D=D, mu=1, s=s))/2**0.5
        )
    elif mu == 'l':
        return qop.Operator(
            space,
...
def basis_change(space, bR, bC):
    ...
    raise NotImplementedError("basis change between {} and {} is not supported".format(bC, bR))
